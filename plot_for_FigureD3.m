...
for j=1:plot_n
    ...
    if k==d
        LogDet1(i,j)=povar(MeeNTLHDm,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
        LogDet2(i,j)=povar(MeeBTLHDm,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
        LogDet3(i,j)=povar(uniform,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
        LogDet4(i,j)=povar(MmLHD,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
        LogDet5(i,j)=povar(MaxPro,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
        LogDet6(i,j)=povar(GMLHD,n,k,Combinations(i,:),plot_theta(j)*0.135,0,eval(['g',num2str(k)]));
    else
        LogDet1(i,j)=povar(MeeNTLHDm,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
        LogDet2(i,j)=povar(MeeBTLHDm,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
        LogDet3(i,j)=povar(uniform,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
        LogDet4(i,j)=povar(MmLHD,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
        LogDet5(i,j)=povar(MaxPro,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
        LogDet6(i,j)=povar(GMLHD,n,k,Combinations(i,:),plot_theta(j)*0.135,nugget,eval(['g',num2str(k)]));
    end   
end
end
a=zeros(1,d);
...
ylabel('Relative APV');
title('Relative APV for Mee-BTLHD','FontWeight','bold');
...
function apv=povar(D,n,k,comb,theta,nugget,g)
pD=D(:,comb);Distance=pdist2(pD,pD)./sqrt(theta*k);
R=(1+Distance+Distance.^2/3).*exp(-Distance)+nugget*eye(n);
num=size(g,1);sigma=zeros(1,num);
for i=1:num
    distances=pdist2(g(i,:),pD)./sqrt(theta*k);
    r=(1+distances+distances.^2/3).*exp(-distances);
    sigma(i)=r/R*r';
end
apv=1+nugget-mean(sigma);
end